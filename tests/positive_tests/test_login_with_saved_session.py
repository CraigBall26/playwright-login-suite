...
# -------------------------------------------------------------------
# This test validates that a previously saved Hudl session allows the
# user to access the dashboard without re-entering credentials.
#
# Real coaches don’t log in every time they open Hudl. Auth0 sessions
# persist, and this test ensures the automation suite can restore a
# valid session and land directly on the dashboard.
#
...
def test_login_with_saved_session(context_with_session: Any, login_data: Any):
    ...
    print(context_with_session.storage_state())
